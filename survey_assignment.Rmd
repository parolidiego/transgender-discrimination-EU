...
Most strong correlations are among variables related race, religion or sexual
orientation discrimination. Consequently, we are grouping them into 3 new
çcomposite variables by averaging related ones, in order to reduce multicollinearity.
...
The resulting data frame has 88 variables, I don't know if we should recode/collapse some factor levels if they have few answers, to reduce the number of variables.

I think it is fine as long as computation times are not too long. If we see that the model take up too much time we can consider reducing the number of variables a priori, but it is better if feature selection is done by the models.
...
Irene: I brought this df here because Brad's the only one if seen using it, 
