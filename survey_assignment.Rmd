...
Given the low number of observations in some categories and the high
number of categories we opt to aggregate some of them, otherwise we risk
too much noise in our models.
...
I thought by aggregating we meant synthesizing the information from the
individual respondents into country-level variables? but I don't know if
this is something that models do directly.
...
Out of all observations Catholic, Atheists/Agnostic, Orthodox Christians
and Protestants sum up to the 81.04% of our sample, being the rest
underrepresented. I believe the warning "Chi-squared approximation may
be incorrect" is because of this. Consequently I don't know if we can
trust the p-value.
...
COMMENT Support doesn't seem to be related to education.
...
COMMENT Support does not vary much by area of residence.
...
dk_fit <- glm(target_NA ~ lgb_discri + gender + age + I(age*age) + antilgbtq_rights + internet_use + social_class + country_name, 
              data = dk_target, 
              family = "binomial")
...
aic_1 <- MASS::stepAIC(dk_fit, scope = list(upper = dk_fit, 
                             lower = dk_fit_null),
        direction = "both", k = 2) # forward based on p-value
...
dk_target2 <- dk_target |> 
  select(target_NA, lgb_discri, age, antilgbtq_rights, internet_use, social_class, country_name) |> 
