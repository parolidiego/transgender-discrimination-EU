...
colSums(is.na(data[dummy_names]))
```

After imputations only the target variable should have NAs. 
...
data_dummy <- data %>%
  ...
  mutate(across(all_of(dummy_names),
                ~ case_when(
                  ...
                  TRUE ~ NA_real_  # assign NA to other unexpected values
                ),
                .names = "{.col}_num"))
```

I converted factors with 2 levels to numerical 0-1 so we can compute their means.
I am creating a column with the proportion of missing answers per country, but if 
we don't need it at this stage we can just remove that line. NOTE THAT the mean of
trans_docs is not considering NAs, just Yes and No answers.

```{r}
# country means
data_dummy <- data_dummy %>%
  group_by(country) %>%
  mutate(trans_docs_na = sum(is.na(trans_docs_num)) / n()) %>% # calculating proportion of NAs
  summarise(
    across(ends_with("_num"), ~ mean(.x, na.rm = TRUE)),
    trans_docs_na = first(trans_docs_na),
    .groups = "drop"
  )

summary(data_dummy$trans_docs_na)
...
```{r}
factor_names <- names(data)[!sapply(data, is.numeric) &
              !(sapply(data, is.factor) & sapply(data, function(x) length(levels(x)) == 2))]
factor_names

# checking the levels
lapply(data[factor_names], levels)
...
data_factors <- data %>%
  ...
  drop_na() %>% # DROPPING NAs TO "SIMULATE" IMPUTATION
  pivot_longer(cols = -country, names_to = "variable", values_to = "level") %>%
  drop_na(level) %>% # Drop rows where the 'level' column is NA
...
I did several tests and still can't figure out why we are seeing NAs in some columns
if I explicitly dropped them (twice). I'll come back to this after imputations to 
see how it changed.

But for now I guess we can join:
```{r}
data_aggr <- data_aggr |> 
...
The resulting data frame has 88 variables, I don't know if we should recode/collapse
some factor levels if they have few answers, to reduce the number of variables.
I'll pull imputations and get back to this (Irene).
