...
GENERAL COMMENTS AND USEFUL CODE ABOUT LOGISTIC
...
Chooses the best model based on the lowest AIC achievable
...
I didn't figure out where the NAs come from, I have to rethink this.
...
Lastly, we join the country-level variables obtained from external sources.

```{r}
data_aggr <- data_aggr %>%
  left_join(country_level_data, by = c("country" = "iso2c")) %>%
  select(-iso3c, -country.y) # drop unnecessary country identifiers
