...
First we will substitute de "Refusal" answers in the remaining variables as NAs. If the respondent refuses to answer it's equivalent to not knowing his or her answer. We exclude `friends_trans` because for that category it could be worth analyzing refusals.
...
The extremely small p-value confirms that the relationship between `religion` and `trans_docs` is highly statistically significant.
...
Summarize individual data to a country level (ex. percentage of population living in rural areas by country, percentage of people from minority x by country). Target variable will become % of people that support the right for transgender people to change their civil documents.
...
We switched to country level predictions because this is as we inteded the task.
