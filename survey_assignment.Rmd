...
Lastly, we can drop the trans_support variable from the table country_level_data,
so it is not duplicated when we join aggregated individual survey data to it.

```{r}
country_level_data <- country_level_data |> 
  select(-trans_support)
...
We realize our new variables are still highly correlated between them. For reducing 
the dimensionality of the dataset even more while preserving information we create
one single variable for general minority discrimination.
...
We decided to exclude white_discri, atheist_discri, christian_discri because we 
realized they had a median of 1 and a quite small mean, meaning there is almost
no reported discrimination against these groups in the dataset.
...
As we have different types of variables we decide to go with the default methods.
Selecting the best method for each type of variable would be too computationally
intensive
...
# We can change m in case we decide to use pool later instead of just completing 1 df)
...
# now run the full model, include age^2 as is good practice
simple_logistic <- glm(trans_docs ~ . + I(age*age), data = datalr, family = binomial(link = "logit"))
...
significant_var <- p_values[p_values < 0.05]
significant_var
...
#### Interpreting the coefficients of these models: 

To interpret the coefficients as odds ratios, anything above 1 indicates they are more likely to oppose the changes to civil documents for trans people. This is because no support are coded as 1s and support are coded as 0s.

```{r}
levels(datalr$trans_docs)
table(datalr$trans_docs)
# Yes = failure = 0
# No = success = 1
```
...
- men (compared to wom)
- younger people 
- people who oppose legal rights to change gender docs (compared to those who do)
...
- people who had a higher discrimination score
...
##### Interpreting log-odds of scaled data
...
Factor variables when exponentialised give the likelihood to respond no/yes relative to the base group in the factor variable. We use the function `factors` to view the levels and identify the base group. For example, if the  to log-odds of A large town, (exp(CommunityLarge town)) = 1.04 and iis 
...
```{r}
# extract the significant variables from the stepAIC model

vars <- all.vars(formula(stepAIC))
# Convert data into a matrix for glmnet
#x_lasso <- as.matrix(datalr[, -which(names(datalr) == "trans_docs")]) 
x_lasso <- as.matrix(datalr[, vars[vars != "trans_docs"]]) 
y_lasso <- as.matrix(datalr$trans_docs)

# Fit the Lasso model (alpha = 1 for Lasso regularization)
lasso_model <- cv.glmnet(x_lasso, y_lasso, family = "binomial", alpha = 1)
...
Here we have the reduced model with lasso. It gives us 8 variables now. We will inc

Brad --> This is a really useful step because the glmer runs so long. But I'm struggling to interpert because the significant variables aren't that intuitive and don't agree as much with the logistic. E.g. variables like gender, 
...
We could run these different models and select the best one: 
null_glmer -> only country random effects for a baseline
glmer_stepwise -> using the best stepwise model best model for individual fixed effects + adding country random effects
glmer_lasso -> using the lasso model fixed individual level effects + country
glmer_level2 -> using the best model from above and adding in all country level fixed effects too
glmer_cross_level -> the model using integrated 
...
LOOCV is a special case of k-fold cross-validation, where k = the number of observations (n). 
...
We should be building a dataframe with 28 rows (1 per country), but
...
In the next steps we will synthesize the information from the individual 
respondents into country-level variables.

Using means for numeric variables:

```{r}
names(select(complete_df, where(is.numeric)))

data_num <- complete_df |> 
  group_by(country) |> 
...
dummy_names <- names(complete_df)[sapply(complete_df, is.factor) & sapply(complete_df, function(x) length(levels(x)) == 2)]
dummy_names

dummy_levels <- lapply(complete_df[dummy_names], levels)
dummy_levels

colSums(is.na(complete_df[dummy_names]))
...
factor_names <- names(complete_df)[!sapply(complete_df, is.numeric) &
              !(sapply(complete_df, is.factor) & sapply(complete_df, function(x) length(levels(x)) == 2))]
factor_names

# checking the levels
lapply(complete_df[factor_names], levels)
...
We are seeing NAs because in not all countries are all the factors' levels represented. For example we see that 
social_class_The higher class of society has 3 missing values. Let's confirm that there are 3 countries in our 
dataframe with no observation for that level

```{r}
complete_df |> filter(social_class=="The higher class of society") |>  pull(country) |> unique() |> length()
```

Therefore those proprtions should be 0s and not NAs
...
The resulting data frame has 28 observations and 80 variables. We might have to
clean the names of the variables that were glued from variable_level.
...
pre_proc <- preProcess(data_aggr, method = "medianImpute")

# Apply the preprocessing
data_imputed <- predict(pre_proc, data_aggr)

# 2. Scale the data (excluding the target and country variable)
# Select predictors, remove country and target variable
dataggr_scaled <- data_imputed %>%
  select(-country, -mean_trans_docs) %>%
  scale()

# Convert to matrix for glmnet
features_scaled <- as.matrix(dataggr_scaled)

# Extract the target variable
target <- data_aggr$mean_trans_docs
...
final_model <- glmnet(features_scaled, target, alpha = 1, lambda = best_lambda)
coef_lasso <- coef(final_model)
print(coef_lasso)

# Calculate predictions and residuals
predictions <- predict(lasso_model, features_scaled)
residuals <- target - predictions

# Calculate R-squared and RMSE
r_squared <- 1 - sum(residuals^2) / sum((target - mean(target))^2)
...
# Optional: if you want to see country-level predictions
# Combine country names with predictions
country_predictions <- data.frame(
  country = data_imputed$country,
  actual = target,
