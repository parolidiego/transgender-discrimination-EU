...
1)  United Kingdom is missing from this dataset as well as from a few
    others in Eurostat (we might have to find it somewhere else).

2)  Data needs further cleaning

3)  Reflect whether we might be better suited with other measures of GDP
    (GDP at PPP)
...
<https://eige.europa.eu/gender-statistics/dgs/indicator/index__index_scores/datatable>
for the year 2019.
...
No UK in this datset (28 rows because it is present a row about EU as a
whole)
...
variales related to energy policies `qb`.
...
because it asks indiviudals for perception at the country level. I think
...
Now we will substitute de "Refusal" answers in the remaining variables as NAs. If the respondent refuses to answer it's equivalent to not knowing his or her answer. 
...
data <- data %>%
  mutate(across(
    all_of(setdiff(factor_variables, "qc19")),  # Excluir "qc19"
    ~ {
      refusal_levels <- grep("REFUSAL", levels(.), value = TRUE, ignore.case = TRUE)  # Buscar niveles que contengan "REFUSAL"
      if (length(refusal_levels) > 0) {  # Si hay niveles que contienen "REFUSAL"
        fct_recode(., NULL = refusal_levels)  # Convertirlos en NA
      } else {
        .  # Si no hay niveles, devolver la columna sin cambios
      }
    }
  ))
...
data <- data %>%
  mutate(
    religion = ifelse(religion == 15, NA, religion),  
    left_right = ifelse(left_right == 4, NA, left_right),  
    marital_status = ifelse(marital_status == 7, NA, marital_status),  
    bill_issues = ifelse(bill_issues == 4, NA, bill_issues),  
    social_class = ifelse(social_class == 8, NA, social_class)  
  )
table(data$social_class)
...
head(data)  # Muestra las primeras filas del dataset para comprobar
```
...
# Función para extraer códigos y valores de variables categóricas
extract_factor_levels <- function(df) {
  tibble(
    variable = names(df),  # Nombre de la variable
    codes = sapply(df, function(col) {
      if (is.factor(col)) paste(seq_along(levels(col)), collapse = ", ") else NA
    }),  # Códigos numéricos (posiciones de los niveles)
    values = sapply(df, function(col) {
      if (is.factor(col)) paste(levels(col), collapse = ", ") else NA
    })  # Valores de los factores
  )
}

# Crear tabla con códigos y valores
variable_levels <- extract_factor_levels(data)
...
First of all, we transform the variable disc_no, which was originally coded in a negative way (1 = "Not mentioned", 2 = "No, you haven’t been discriminated against"), into a positive dummy variable to make its interpretation more straightforward.

```{r}
str(data)
data <- data |> 
  mutate(suffered_discr = ifelse(disc_no == 1, 1, 
                                 ifelse(disc_no == 2, 0, NA))) |> 
  select(-disc_no) 
...
We should be checking whether there are other levels for different
variables that need special treatments (such as refuse to respond,
should we turn those into NAs?). We can do this by looking at the
summary below

First let's change the name and try to clean them to get to significant
names
