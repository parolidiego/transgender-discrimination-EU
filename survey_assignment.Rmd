...
For the right wing x lgbti policy variable, as the coefficient is negative, we infer that the effect of being right wing is weaker in countries with higher lgbti policies. So more mild right wing opposition to transgender documenation occurs in more progressive countries. This makes sense and maybe helps capture some of the political variance between countries. 

For the women x lgbti policy variable, suggests that the effect of being a since it's positive, we conclude that the effect of being a women in countries with stronger lgbti policies is stronger. 

For non-believer x democracy index and gender inequality index, being negative, this implies that the effect of being a non-believer is weaker in countries with with higher democracy score and also more gender inequality (as this is a negative scale, greater equality comes with lower scores). This could align with more progressive democratic countries having lower levels of religion and higher support for the target variable. 
...
summary_table <- prediction_summary %>%
  ...
  group_by(country) %>%
  summarise(
    actual_rate = mean(trans_docs),
    null_rate = mean(null_model_pred),
    m1_rate = mean(m1_model_pred),
    m2_rate = mean(m2_model_pred),
    m3_rate = mean(m3_model_pred))

#plot all of the model estimates on a graph
# with unpivoted data
summary_table |> 
          ggplot(aes(x=reorder(country, -actual_rate), 
                     y = actual_rate))+
          geom_col()+
          geom_point(aes(y = null_rate), 
                     color = "blue", size = 3, shape = 18,
                     position = position_nudge(x = -0.3)) +
        geom_point(aes(y = m1_rate), 
                   color = "red", size = 3, shape = 15, 
                   position = position_nudge(x = -0.1)) +
        geom_point(aes(y = m2_rate), 
                   color = "green", size = 3, shape = 16, 
                   position = position_nudge(x = 0.1)) +
        geom_point(aes(y = m3_rate), 
                   color = "purple", size = 3, shape = 17,
                   position = position_nudge(x = 0.3)) +
          theme(axis.text.x = element_text(angle = 45, hjust = 1),
                            axis.title.x = element_blank()) +
          scale_y_continuous(labels = scales::percent_format(scale = 1)) + 
          labs(y = "Proportion of support, by mixed model type")
